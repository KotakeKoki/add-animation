...
$('.animated').waypoint({
  handler(direction) {
    if (direction === 'down') {
      $(this.element).removeClass('fadeOutUp active');
      $(this.element).addClass('fadeInUp active');
    }
  },
  /**
...
$('.animated').waypoint({
  handler(direction) {
    if (direction === 'up') {
      $(this.element).removeClass('fadeInUp active');
      $(this.element).addClass('fadeOutUp active');
    }
  },
  /**
   * 要素の上端が画面のどの位置に来たときにhandlerメソッドを呼び出すか指定
